...
config_layout = [[sg.Text('Select Initial Board Layout', text_color="yellow")],
                 [sg.Radio(board, 1, default=True) if board == 'Standard' else sg.Radio(board, 1) for board in
                  board_choice],
                 [sg.VerticalSeparator(pad=((0, 0), (10, 20)))],
                 [sg.Text('Select Player Color', text_color="yellow")],
                 [sg.Radio(color, 2, default=True) if color == 'Black' else sg.Radio(color, 2) for color in
                  player_color],
                 [sg.VerticalSeparator(pad=((0, 0), (10, 20)))],
...
selected_board = constant.DEFAULT_BOARD  # TO_BE_UPDATE: NEED TO CHANGE BASE ON USER SELECTION
...
if event == "Exit":
    ...
    game_layout = [[graph_element, sg.Column(game_info_layout)]]
